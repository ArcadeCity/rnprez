...
function Items({ w = 0.7, gap = 0.15 }) {
  ...
  // Load urls via expo-asset and get their localUri
  const [images, setImages] = useState([])
  useEffect(() => {
    Promise.all(
      urls.map((url) => Asset.fromModule(require(`../assets${url}`)).downloadAsync())
    ).then((assets) => setImages(assets.map((asset) => asset.localUri)))
  }, [])
  ...
  return (
    <ScrollControls horizontal damping={10} pages={(width - xW + urls.length * xW) / width}>
      ...
      <Scroll>
        {
          images.map((url, i) => <Item key={i} index={i} position={[i * xW, 0, 0]} scale={[w, 4, 1]} url={url} />) /* prettier-ignore */
        }
      </Scroll>
    </ScrollControls>
  )
